...
POST /analyze  – Accepts a PDF resume + job description text;
                 returns a TF-IDF cosine similarity score + skill gap analysis.
"""
...
def _process_uploaded_pdf(resume_file) -> str:
    """Save the uploaded PDF securely and extract its text."""
...
def analyze():
    ...
    try:
        with open(ideal_resume_path, "r", encoding="utf-8") as f:
            ideal_text = f.read()
        comparison = compare_resumes(resume_text, ideal_text, resume_skills)
